...
## ▶ STATUS: v0.18.3.259 BAT PASSED (Aaron: "Much better!") — **PUSH CANDIDATE**. B-Garden→Balamb 11.3km in 23s, first attempt; inside the 1200u vehicle final-approach circle: 53 ARC / 6 FINAL / 0 PIVOT / 0 REVERSE — entry-aim orbit GONE (#68 comment posted). Engine vehicleId=33 first-tick detection confirmed again. Last wedge burst was 3km out. Aaron pushes via Utilities/push_to_github.ps1 (CHANGELOG heading == 0.18.3.259 ✓; .254 = pushed HEAD c6e758d, .255–.259 local). Remaining #68 vehicle item: mid-route terrain scrapes on the pass legs (6 bursts clustered dist 3000–3900 from Balamb, same corridor as .258 idx 84/95) — route-planning clearance at car scale, likely offline work per project rule. #68 on-foot end-game items (steer-target oscillation, chain-advance stall) also still open.
