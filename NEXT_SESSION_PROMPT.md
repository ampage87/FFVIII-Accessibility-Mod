...
**GitHub HEAD = v0.18.1.3 (`af7bf5b`) — Ability Build 1 PUSHED. Local working tree = v0.18.1.13, BAT-CONFIRMED and READY TO PUSH (NOT yet pushed):** the ENTIRE refine flow now speaks — Build 2 item list, 2b "Refinable / Cannot be refined" tag, Build 3 recipient picker (now with "has N <Magic>" stock on the 400 ms beat), Build 4 quantity selector — plus all the v0.18.1.10–.13 polish (entry-blip gate, double-announce fix via `+0x2E7`/`+0x2E9`, savemap magic-stock read). All diagnostics off. See DEVNOTES.md top block for the live status. **NEXT ACTION: Aaron runs `Utilities/push_to_github.ps1`** (one push carries 2b + 3 + 4 + all fixes; Claude NEVER pushes), then Claude verifies HEAD via `github:list_commits` and updates issue #42. Confirmed offsets in DEVNOTES (recipient `+0x2DE`, quantity `+0x2E5`, sub-phase via memory `+0x2E9`/`+0x2E7`; savemap char structs `+0x048C`+id*152, Magics[32] at `+0x10`, Water=spell id 10). Remaining verify items: multi-input recipes (X→Y, X>1) total math; non-Water `MAGIC_NAMES` entries (only Water is in-game-verified — fix any wrong count as reported); status-magic ids 41+ (name-only until confirmed); renameable Squall/Rinoa names. The rest of this file is the original pre-discovery #42 plan, kept for reference.
