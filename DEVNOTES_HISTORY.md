...
## v0.18.3.305 (#109 axes cache + #110 camera-angle survey) — moved out of DEVNOTES.md at `.307`

**Prior (v0.18.3.305 — BAT'd, ALL THREE CONFIRMED).** **#109 cache WORKS**: one calibration on `gpbig1a`, then `cached axes re-applied on field load` on **four** later loads with no relearn — Aaron: *"it did need to calibrate on the left side, but after that its directions seemed accurate"*. **#110 first verdict matches the prediction**: `gpbig1a rawCA=+38.32 quantizerChose=+0 alternative=+90 measured=+90 → ALTERNATIVE CORRECT`. **But that is the field the hypothesis was DERIVED from — consistency, not independent confirmation.** `gpgmn3`/`gpbrdg1`/`gppark1` unvisited; **the threshold change stays unshipped** (93 fields). `gpbig2a` (flipBand=0) never needed correcting — consistent but weak. #107/#108 unchanged. Full chapter in `CHANGELOG.md`; survey in `claude/CAMERA_ANGLE_SURVEY.md`. **The .304 calibration is correct; it just does not survive the next floor.** `.304` BAT: Aaron said it *"seems to be working"* but *"you have to approach the downward stairs in a particular direction, and the mod was getting a bit hung up on that"*. **The log settles it without inference:** in one 4.5-minute walk `gpbig1a` loaded **EIGHT times** (plus three `gpbig2a` loads) while the calibration fired **twice** — `ResetFieldNavState` clears it on every load — and the two lists correlate exactly. `12:27:14 load → 12:27:22 calibrated →` the 12:27:28/29/45/51 drives are the ones Aaron said worked. `12:28:10 load → never recalibrated → 12:28:44 drive → 20 recovery attempts → "Gave up. Distance remaining: 999."` `12:29:07 load (floor 4) → never recalibrated → 12:29:08 drive → wedged AT the stairs, 56 identical position samples.` **Why it FEELS like an approach-direction problem:** with the axes wrong the drive presses the WRONG ARROW KEYS, so whether the player makes progress depends on how the axis error happens to line up with the approach. The staircase is not fussy — the steering is rotated. Caught in the act at 12:29:09: the drive wanted world (−53,−110) (west-south), which with the true mapping (screen-up = world −X, screen-right = world +Y) is **UP-LEFT**, but on stale `ca-quantized` axes it pressed **DOWN-LEFT** = world (+X,−Y), the opposite in X. Player froze at (−2154,−100), **12 units short of crossing line1**, and stayed there. **FIX = cache corrected axes per fieldId, re-apply at field load** (`CamAxesCacheStore`/`CamAxesCacheLookup`, 24 entries, round-robin). **Placed in `field_nav_helpers.inl`, NOT `field_navigation.cpp`** — that file is at 81,792/81,920 and the rule is split-before-landing, not shave-to-fit; helpers.inl is included ahead of both `fieldscripts.inl` (applies) and `observe.inl` (fills). **INSEPARABLE SECOND PART — the one-shot lock becomes a per-load CAP of 3, and `empirical-cached`/`empirical-corrected` join `ca-quantized` as contradictable sources.** This is what makes cacheing safe: **`.304` applied one correction that was 180° WRONG** (12:25:38 arrow=DOWN consensus (−0.872,0.490)=151° → camRight=(0,−1), vs 12:27:22 arrow=UP consensus (−0.995,0.100)=174° → camRight=(0,+1)). Opposite keys cannot both move the player the same way, so one was a wall slide and DOWN was the bad one. Under a hard lock that stood for the visit; cached unchanged it would stand for the SESSION. A 180° error diverges far past 45°, so it self-repairs on the next clean sample. **SEEN, NOT FIXED (filed against #100):** the wedge recovery repeats an IDENTICAL nudge — same triangle pair, same direction, same 8 ticks — up to 20 times (`tri 10→54 dir=(0.96,0.28)` ×10; `tri 75→74 dir=(−1.00,0.02)` ×10). A deterministic action that failed cannot succeed by repetition; it burned 38 s before giving up. Fixing the axes should remove most wedges. **BAT: walk several floors and drive to the stairs on more than one.** Expect `[NAV-CAL] ... cached axes re-applied on field load` from the SECOND entry to each shaft field onward, no relearn wait, and the drive behaving on every floor the way it did inside `.304`'s calibrated window. If a field ever calibrates wrongly, a second `[NAV-CAL] ... unlocking correction` should overwrite it rather than it sticking. **ALSO FOLDED IN — #110, LOG-ONLY, ZERO BEHAVIOUR CHANGE.** Aaron asked whether the field data itself could give us the orientation at load instead of a multi-session diagnostic. **It can: I extracted the `.ca` camera record for ALL 894 fields straight out of `field.fs`** → `offline/CAMERA_ANGLES.csv` + `offline/CAMERA_ANGLE_SURVEY.md`, parser validated against the mod's own `[CA]` log lines (gpbig1a 38.32, bgryo1_4 46.60, bghall_1 7.73, bg2f_1 65.38, bgroom_1 −62.49, bghall_4 23.80 — all exact). **(1) FF8 cameras are NOT near-axis-aligned** — the `.17.5` quantizer was justified on four fields that "round cleanly to a world cardinal", but across all 894 only **35% sit within 5° of a cardinal and 44% discard more than 20°**. The snap is making a large correction across most of the game. **(2) The threshold is NOT 45°.** gpbig1a's measured response (RIGHT → world 84°, DOWN → world −5.7°, two independent arrows) gives camRight ≈ 90° — matching neither the quantizer's 0° NOR the raw 38.32°, so the engine does quantize, with a lower threshold. Against the known-good fields the bracket is **(23.8°, 38.32°]**. **NOT shipping that change:** it would flip **93 fields**; every field known to work today is outside that band and gpbig1a is inside it (strongest evidence it is safe) but 93 fields is far too large a blast radius for ONE confirmed data point. **Shipping the measurement:** `[CAM-SNAP]` at every field load (raw angle, both candidates, flipBand flag) and `[CAM-SNAP-VERDICT]` when a calibration lands (engine agreed with the quantizer's pick / the rejected one / neither). Every field that calibrates anywhere in the game is now a data point. **PREDICTIONS ON RECORD:** `gpgmn3` (+38.80) and `gpbrdg1` (+129.10) → ALTERNATIVE, same as gpbig1a; `gppark1` (−46.80) and `gpbig2a` (−24.76) → QUANTIZER. **If gppark1 or gpbig2a also says ALTERNATIVE the model is wrong and no threshold change ships.** **SEPARATE BUG, filed not fixed:** 45 fields carry >1 camera and `LoadCameraAxes` reads camera 0 ONLY; **18 have cameras that snap to different cardinals**, so a script-driven camera switch puts the axes a full cardinal out silently — `glwitch1`, `glstage1`, `ggroom1` are on that list and all three appear in the sewer/gate work.

---

## v0.18.3.304 (#107 + #108) — moved out of DEVNOTES.md at `.305`

**Prior (v0.18.3.304 — BAT'd, both fixes CONFIRMED: `[NAV-CAL]` fired and corrected the axes, and the bounded arrival test rejected 8 false "Arrived" claims at t=4.2–4.4. What remained was that the correction did not survive field loads — see .305).** `.303` BAT: **the stairs list correctly now, but Aaron could not GET to them**, and neither reason is the catalog. The entry position `(-2150,-197)` is corroborated (every `.301` floor change fired from `(-2400,-470)`, right beside it); both problems are downstream of it. **Two changes because neither is testable without the other** — fix the false arrival and the drive still steers on 50°-wrong axes and wanders; fix the axes and it still stops short and says it arrived. Different files, different log tags, one walk. **#107 FALSE ARRIVAL (`field_nav_autodrive.inl`):** the trigger-line arrival test was the **INFINITE-LINE side test** — the **FIFTH** site of that bug and the first outside a catalog filter (`.17.8.10` gateways, `.268` interactions, `.278` exits, #94 events were all bounded; this one was missed). `gpbig1a line1` is `(-2141,-89)→(-2158,-305)`: 217 units and almost parallel to Y, so its infinite extension is a wall across the whole map at X=−2150 and ANY point past that X flips the sign. Log: start `(-2221,-1089)` crossNow=−280; at `(-2162,-1161)` crossNow=**+13688 → "Arrived."**, keys released, **~960 units from the staircase**. In a catalog filter this bug flickers a row; here it is a confident spoken claim that the destination has been reached, which a blind player has no way to catch. Fix = sign flip **AND** player alongside the segment (project onto the line, foot of the projection within it, 15% end margin). Failing the bound is the SAFE failure — the drive keeps walking and the engine's own MAPJUMP ends it. **#108 CAMERA AXES (`field_nav_observe.inl`):** `[NAV-OBSERVE]` logged **20+ samples on gpbig1a diverging 48–84°**, never converging; `gpbig2a` the same. That is why the stairs said "north". Root cause = the 90° CA quantizer: `camRight angle=+38.3deg -> snap=+0deg`, only **6.7° from the 45° coin-flip boundary**, discarding 38° (`gpbig2a`: −24.8→0). **NOT fixing the quantizer** — the raw CA does not match the engine either (measured screen-up ≈145°, raw predicts ≈128°, quantized 90°), so "use the raw value" trades one wrong answer for a less wrong one. **The real defect:** the closed-loop empirical calibrator built for exactly this was hard-gated to `s_camAxesSource=="identity"`, so a `ca-quantized` field could NEVER be corrected however badly the engine contradicted it — it watched all 20+ samples and was forbidden to act. Now a `ca-quantized` field becomes eligible once the engine contradicts it: **triggering sample AND consensus must both diverge ≥ 45°**. 45 is not tuned, it is the **cardinal sector boundary** — below it the spoken cardinal is still right so there is nothing to fix; at or above it the cardinal is provably wrong. **REGRESSION SAFETY (this path can now touch working fields):** the gate MEASURES HARM on this field rather than guessing which fields look risky — correct axes produce consensus agreeing with the prediction, gate stays shut, nothing written. I first tried gating on "the quantizer discarded a lot of angle" and **rejected it: `bgryo1_4` discards 43.4° and navigates correctly**, so that test would have fired on a healthy field. **BAT: walk a few steps with the arrows on a shaft floor, then open the catalog and drive to the down stairs.** Expect `[NAV-CAL] ... unlocking correction` + spoken "Camera calibrated.", directions matching the screen, and the drive either reaching the stairs or continuing — **never "Arrived" in open floor**. **SAY IF ANY OTHER FIELD STARTS GIVING WRONG DIRECTIONS** — that is the regression this gate exists to prevent, and `[NAV-CAL] ... NO correction applied` lines show it staying shut where it should.

---

## v0.18.3.303 / v0.18.3.301 / the FLOOR ANSWER — moved out of DEVNOTES.md at `.305`

(All three are consolidated and expanded in the project doc `claude/DDISTRICT_PRISON_SHAFT.md`.)

**Prior (v0.18.3.303 — BAT'd, stairs BOTH list correctly; what failed was getting to them, see .304).** **The DOWN staircase now lists too.** `.302` BAT: up stairs correct and correctly placed, down stairs absent entirely. **The log named the culprit — no guessing was needed.** `[refresh] STAIRS line2 ... higher=0 lower=1` — that count comes from my own comparison loop walking `s_capturedLines`, so it had **found `line1`, read its Z and counted it**: present, active, SCREEN_BOUND, sharing the destination. Yet `line1` produced no STAIRS line and still came out as `Interaction 1`. One filter fits all three facts and the log states it outright: `[JSMScan] REQ-interact: Line ent1 'squall' ... hasDialogReqTarget=1`, and `[LINE-PAIR]` maps `gpbig1a line1 → jsm1 'squall'` while `line2 → jsm2 'zell'` (no REQ target). **That is the whole asymmetry between the two staircases:** the UP stairs are a plain walk-across line; the DOWN stairs run a **scripted descent with dialogue** (the same script that answers "(No sense going back up.)"), so the down line legitimately REQs a dialog-bearing entity and got classed dual-purpose. The dual-purpose rule — *if it talks it is an Interaction, not an Exit* — is right for a dormitory bed and **wrong for a staircase, which is the only way off the floor**. **Two edits, both gated on "prison-shaft self-destination line" so beds and every other dual-purpose line are untouched:** (1) `field_nav_catalog_triglines.inl` — hoist the shaft-stair test **above** the `hasDialogReqTarget` `continue` and exempt shaft stairs, logging when the exemption fires; (2) `field_nav_catalog.inl` — the Interaction block has **TWO** branches setting `isInteractive` and `.302` guarded only the second (self-loop); the down stairs took the **first** (`hasDialogReqTarget`) and kept emitting `Interaction 1`. **This is the DEVNOTES rule "when a fix targets one of several passes writing the same field, check ALL of them" — I wrote that rule and then broke it in `.302`, which is why this needed a second cycle.** **BAT: on a shaft floor, OPEN THE CATALOG.** Expect BOTH `Stairs down to Floor N−1` and `Stairs up to Floor N+1`, **no** `Interaction 1`, and two `[refresh] STAIRS` lines — `line1` `higher=1 lower=0` → down, `line2` `higher=0 lower=1` → up.

**Prior (v0.18.3.301 — BAT'd, all three confirmed).** Floor announcement (#90) **CLOSED** — "Floor 4, left side"/"right side" on every change including same-fieldId ones. Gateway split + "Top crossing"/"Bottom crossing" (#91 R3) confirmed, Aaron says they read correctly so **larger-Y-is-top is now established, not assumed**. Catalog dump (#92) **CLOSED** — `GATEWAY grp` rows finally printed, counts match rows. **THE GEOMETRY, from Aaron:** `gpbig1a`/`gpbig2a` are the LEFT and RIGHT halves of ONE circular walkway around a central hole, joined at the TOP and BOTTOM of the circle; that is why 10 crossings gave 0 floor changes.

**★ FLOOR ANSWER (settled `.299`, both #89 and #90).** The displayed prison floor is the **dialog slot `FIELD_VAR_TABLE_BASE[0x20]`** — read off two auto-captured screenshots ("Floor 6" @ slot=6/vb=5, "Floor 4" @ slot=4/vb=3). **Varblock `0x01B5` (437) is floor MINUS ONE**, so the announcement needs `vb + 1`. Fix not yet written: `field_announce.cpp` `:127`–`:135`, append the floor for shaft fields and key already-announced on `(fieldId, floor)` — the indicator window is PERSISTENT (190+ renders/floor, updates its digits in place), so the announce must key on the VALUE changing, not the window appearing. Do NOT edit `field_display_names.h` (`strstr` dedupe, `field_archive.cpp:735-741`).

---

## v0.18.3.302 and the v0.18.3.298 BAT review detail — moved out of DEVNOTES.md at `.304`

**Prior (v0.18.3.302 — BAT'd, half-confirmed: up stairs YES, down stairs NO — fixed in `.303`).** **The prison stairs are in the catalog, named by direction and destination floor.** Aaron's clue closed it: the game answers **"(No sense going back up.)"** when you cross the up line, and that plus the SETLINE geometry identifies both staircases. `gpbig1a` has TWO self-destination SCREEN_BOUND lines and BOTH were lost — `line1` surfaced as the meaningless "Interaction 1", `line2` not at all. **`line1` (-2150,-197) z=−61 = DOWN; `line2` (-2276,269) z=+371 = UP.** Confirmed twice: all three `.301` floor changes fired from (-2400,-470), just past line1, floor decreasing; and the refusal came from the NORTH line. **RULE = higher-Z self-destination line goes UP** — the same discriminator I withdrew in `.298` when INF gateway `lineZ` came back (0,0); SETLINE heights are real and ~430 apart here, about one floor. Same idea, correct data source. NOT keyed on line index or SYM ('squall'/'zell', meaningless) so it carries to other shaft screens; direction claimed ONLY when a counterpart exists to compare against — a lone line or a tie gets bare "Stairs", never a coin-flip. **Also (R4): stairs are EXEMPT from the bounded separation test** — gpbig1a's west wall has FIVE lines within ~500 units so whichever you are not next to reads as "separated", which is exactly how line2 vanished. That test has now false-positived in FOUR blocks (`.17.8.10` gateways, `.268` interactions, `.278` exits, #94 events); losing the only way off a floor beats listing a staircase you walk around to. Interaction block no longer promotes shaft self-loops → no duplicates; the dorm bed and every other self-loop field untouched. New `ReadShaftFloor()` in `field_nav_helpers.inl` (varblock `0x01B5 + 1`). **BAT: open the catalog on any shaft floor** — expect "Stairs up to Floor N+1" AND "Stairs down to Floor N−1" as two entries plus a `[refresh] STAIRS` line each showing meanZ and higher/lower counts. Walk to each; **if swapped, the Z sign is inverted and it is a one-line flip.** The up stairs may refuse with "(No sense going back up.)" depending on story state — that is the game, not the label.

**Prior (v0.18.3.298 detail).** Full review of the 2026-07-31 Winhill + prison session → `claude/BAT_REVIEW_2026-07-31.md` (project) + **18 issues, #89–#106**. All four of Aaron's reports confirmed, three worse than reported. Ships **only the two read-only probes** at the top of the review's order (combined — both are pure observation): **`[VAR-DROP]` (#89)** the floor indicator is `"Floor " + 0x04 param 0x30` and `FieldExpandRawVars` drops anything outside `[0x20..0x27]` (also eats the draw-point quantity) — logs the param, the dword the engine WOULD read at `0x01D2B4B0 + param*4`, and `table[0x20..0x3F]`, throttled per-param/2 s. **`[FLOOR-PROBE]` (#90)** the shaft changes floor WITHOUT changing fieldId — 23 transitions were 795→795 and `FieldAnnounce` bails on `curId == lastAnnounced`, so they announced NOTHING; prison-gated byte dump of varblock `0x0190-0x01CF`, **at MAPJUMP not in `RefreshCatalog()`** (that placement cost two BATs on #85). **`[INF-GW-Z]` (#91)** `LoadINFGateways` read both exit-line Zs and discarded them and never parsed `destinationPoint` at +12; now on `GatewayInfo`, logged on a SEPARATE line so the old `[INF-GW]` line stays byte-comparable. **Dump fix (#92)** the dump tested `<= -300` before `-400`, so EVERY gateway exit printed nothing — silently, in every log this project has collected; now keyed on `gatewayIdx`, unknown sentinels print as UNKNOWN. **BAT:** ride the lift, stop on 3-4 floors, F11 each, then walk both staircases on one shaft screen. Expect `[VAR-DROP]` "would read" == the screenshots, `[FLOOR-PROBE]` moving, `[INF-GW-Z]` opposite `dz` signs on the two same-destination gateways, `GATEWAY` rows in the dump. **If `[VAR-DROP]` does NOT match, the resolver for params >= 0x28 is elsewhere — chase it, do NOT widen the range** (a confidently wrong floor is worse than none). g++ compile-clean on the extracted probe (run, not just compiled). `field_navigation.cpp` untouched. **Next:** #93 → #94 → #97 → #99.

---

## v0.18.3.299 and v0.18.3.298 (diagnostics) — moved out of DEVNOTES.md at `.303`

**Prior (v0.18.3.299 — DIAGNOSTIC-ONLY).** `[FLOOR-SHOT]`: the mod takes the screenshot itself. `.298` found the floor number in TWO places disagreeing by one — display slot `FIELD_VAR_TABLE_BASE[0x20]` (6→5→4) and varblock `0x01B5` (4→3→2) — and picking wrong makes every floor announcement off by one. **I asked Aaron to F11 while the Floor window was up; he can't, and the reason IS this bug**: the window renders 190+ times per floor but decodes to the bare string "Floor" every time, so the dedup eats every repeat and he heard it ONCE for a three-floor descent. He can't hear when it's on screen, so he can't time a keypress to it. `FloorShotProbe()` fires from the show_dialog text path matching the RAW bytes `4A 6A 6D 6D 70 20`, placed **before** the hash/decoded dedups (the raw buffer is byte-identical every floor — the engine substitutes digits at render, they're never in the text). Rate-limited on the VALUE not the frame (one shot per floor) and **both candidates go in the filename** (`floor_HHMMSS_mmm_s20-5_vb-4.png`) so the answer is readable off the directory listing. Async + 30-frame delay (Scan UI's mechanism) since it runs inside a MinHook callback. Prison-gated, behind `FLOOR_SHOT_PROBE`. g++ compile-clean and behaviour-tested (fires / dedupes / ignores non-Floor / re-fires on change / ignores non-prison). **BAT: walk any shaft floors, AND press `-`/`=` once on a shaft screen** — the catalog was never opened in the `.298` run so #92 and #91's R1/R2/R3 are still untested (third time this project has lost a probe to that).

**Prior (v0.18.3.298 — DIAGNOSTIC-ONLY, BAT'd 2026-07-31 20:42).** Results are on the issues; headlines: **#89 solved** — the floor is at slot `0x20`, NOT the `0x30` the param names; slot `0x30` reads 0, so blindly widening `FIELD_VAR_PARAM_MAX` would have announced **"Floor 0" on every floor**. Logging first was load-bearing. **#90 solved** — varblock `0x01B5` (437) is the counter; `0x01A5`=0 and `0x01B4`=29 constant. **#91 REFUTED** — gateway `lineZ` is `(0,0)` everywhere, arrive-Z is 16/74/45/78 all same sign, so Z cannot label up/down; **and the model was wrong** — `gpbig1a`/`gpbig2a` are the two halves of ONE floor, the eleven gateway crossings change nothing, and the floor changes only on `MAPJUMP3` 795→795 (`inline_param=16`). That makes **R1 (the self-loop suppressor) the whole story** for the missing stairs. **#92 untested.**

---

## v0.18.3.301 (detail) and v0.18.3.300 — moved out of DEVNOTES.md at `.303`

**Prior (v0.18.3.301 detail — THREE separable units, each with its own log tag; deviation from one-change-per-cycle flagged deliberately, all three ride the same walk).** **THE GEOMETRY, from Aaron, which reframed everything:** `gpbig1a`/`gpbig2a` are the LEFT and RIGHT halves of ONE circular walkway around a central hole, joined by a crossing at the TOP of the circle and one at the BOTTOM. INF corroborates exactly — two gateways per half at mirrored heights (y ≈ +1800 / −1850) — and it explains 10 crossings / 0 floor changes: they are LATERAL. **(1) #90 floor announcement (headline).** The shaft changes floor WITHOUT changing fieldId so the fieldId-change trigger never fired and 23 changes announced nothing. Announce key is now `(fieldId, floor)`; floor = varblock `0x01B5 + 1`. Ring halves DROP the archive ordinal and speak **"Floor 4, left side" / "right side"**; other shaft fields keep their name and gain ", Floor N". **(2) #91 R3 gateway split + crossing names.** Grouping on destFieldId alone merged two exits 3,644 units apart into one midpoint entry (open floor, auto-drive walked to it). Now also requires within 800 units (genuine dupes < 200). Split pairs need distinct names or the `.270` exact-name dedupe drops one → **"Top crossing" / "Bottom crossing"**; a floor label would be WRONG here since both stay on the same floor. From the gateway's own Y, not the camera. **⚠ ASSUMPTION: larger Y = top. The sign is NOT documented anywhere and I did not verify it — both centres are logged, a swap is a one-line flip.** **(3) #91 R1 groundwork, diagnostic only:** `[MAPJUMP-HOOK]` now logs player XY at fire time. `gpbig1a` has TWO self-dest lines ~470 apart on the west wall (`line1` (−2150,−197), `line2` (−2275,269)); one is up, one is down, and nothing static separates them — gateway Z is (0,0) everywhere and nav_data logs only TRIANGLE changes so it went quiet 13–49 s before each jump. **NOT in this build: naming the stairs** — needs R1 + the reachability drop that swallowed line2 (R4), and until the diagnostic lands both would read "Stairs" with no way to choose. Next cycle, as "Stairs to Floor N". **BAT: walk the shaft with `-`/`=` on BOTH halves.** Expect the floor announcement on every change including same-fieldId ones, two crossings instead of one phantom, `INF-GW gw SPLIT` + `ring crossings named` lines, and the first-ever `GATEWAY grp` rows (#92). **SAY IF TOP/BOTTOM ARE SWAPPED.**

**Prior (v0.18.3.300 — FIRST BEHAVIOUR CHANGE of this arc, ONE change).** **#91 R2: the stale-gateway rule now also requires `s_gatewayCount <= 1`.** The `.299` BAT caught it deleting a REAL exit: `gpbig2a` (Prison 5) offered exactly TWO catalog entries on all five visits — a cell door and the Directory — and **no listed way back to the floor**, which Aaron then walked through anyway 30 s later. Premise inverted: the screen has ONE screen-bound line, it points at a **cell** (965), and that single unrelated line arms `fieldHasLineExits` which deletes the only real exit. The shaft uses lines AND gateways for DIFFERENT exits — a shape the rule never contemplated. Fix = scope the heuristic to the evidence it was established on (same move `.291` made rescoping addr-as-literal to `bg*`): it was derived from and only ever needed by **glfurin1, which has ONE gateway**; every other field in its own evidence block resolves zero line exits so it never fires there, and `glwitch1`'s gateway dest equals its line dest so it's kept either way. **Provably inert on the whole documented evidence base**, preserves glfurin1, fixes both prison screens (2 gateways each). MUST be `s_gatewayCount` (raw) not `s_dedupGatewayCount` — the prison's two gateways share a dest and merge to ONE group. **Direction of risk: pure narrowing — it can only ADD exits, never remove one.** Logs `[refresh] stale-gateway rule NOT applied` when the gate engages. **BAT: walk the shaft with `-`/`=` on BOTH screens.** Expect `gpbig2a` 2→3 entries gaining "Exit to Prison 3", `gpbig1a` gaining "Exit to Prison 5", the gate line on each, and the **first-ever `GATEWAY grp` row** (#92 — still never executed, because this rule deleted every gateway before the dump could print it) reading `merged=2`, which is also R3's first direct evidence.

---

## v0.18.3.297 — #85 state exclusion ENABLED (moved out of DEVNOTES.md at `.298`)

Full-maze `.296` BAT: the refined rule was correct on **all six** verdicts across both glwater3 states (standing byte=0 → `ladline5` SUPPRESS, `ladline6`+`saku3` KEEP; fallen byte=9 → all KEEP); the naive rule would have deleted Gate 3 in both. **glwater2 (Sewer 3) still gave no verdict** — entered twice, catalog never opened; **2nd BAT lost to that**, and it was the probe's placement inside `RefreshCatalog()`, not Aaron's testing. (The `.298` probes were deliberately placed at MAPJUMP fire time for exactly this reason.) Closed structurally rather than spending a 3rd cycle: the **ANCHOR GUARD** means glwater2 can never anchor (every observed read of 340 is 0 or 9; it wants 25/13/16) so it is *provably* untouched. **`field_navigation.cpp` went 63B over the hard fail; I trimmed MY OWN new comment, not existing explanation. 81,792/81,920 — split it (#37) before anything else lands there.** CHANGELOG `.297`. **BAT was:** gate room — expect ONE FEWER "Object" in the standing state, all Gates present, `[STATE-GROUP] ... ANCHORED` + `[SUPPRESSED]` on `ladline5` only; then walk the whole maze and confirm no gate went missing anywhere.
...
## 2026-07-11/12 — #70 world-map chapter close-out, v0.18.3.212–.225 (moved from DEVNOTES 2026-07-13; play-by-play in CHANGELOG `## v0.18.3.212`–`## v0.18.3.225`)

- **.225 release prep:** `world_map_drive.inl` (156 KB) split → `world_map_drive_helpers.inl` + `world_map_drive.inl` + `world_map_drive_exec.inl`; `world_map_planner.inl` (96 KB) → + `world_map_planner2.inl`. Byte-identical textual-include splits; every src file back under 80 KB. `AUTOTEST_CMD_ENABLED` gated to 0.
- **.223/.224 route-around escape:** `EscapeSteerAround` routes AROUND a location's padded firing box toward the first ON-ROUTE waypoint outside it; resume at the on-route escape-target index (`s_driveEscapeTgtIdx`), never a waypoint behind. VERIFIED LIVE: full Galbadia loop GG→Timber→Dollet→Timber→GG, all 4 legs arrived; Balamb regression PASS (BG→Fire Cavern).
- **Escape mechanism:** `ArmFiringAreaEscape` (StartAutoDrive + .220 deferred resume): EXCURSION SKIP past leading in-area waypoints (EA_PAD=192); STEER-AROUND the padded box (EA_STEER_ARM=768); resume at that on-route index; target's own area exempt. Heritage .221→.222→.223/.224.
- **Automation loop (LOCAL, gated off at .225):** live log tailing via `_fsopen(_SH_DENYWR)` (.212); `src/autotest_cmd.inl` polls `Logs/autotest_cmd.txt` for KEY/WAIT/SHOT, injects via ChaseKeyboard DIK overlay + VK SendInput (.213–.218), acks `[AUTOTEST]`. JAWS must be exited; arrows only arrive via the overlay.
- **Nav bugs the loop found+fixed:** .216 stale watchdog clocks across battle-resume (`s_driveWatchdogGen`); .217 `GetWorldMapPosition_Active` foot-motion override (stale vehicle byte → wrong-continent car_pos); .219 routenet 4-plan cap counted battle resumes; .220 resume replan deferred 20 ticks.
- **Aaron's test cases — ALL PASS on .225** (Slot2Save1 = outside GG; Slot1 Block 3 = Balamb–Alcauld Plains): full Galbadia loop (4 legs, field↔world-map exits via catalog `=`/`-` → "Exit to World Map" + `\`, or DOWN-walk out at Timber) and Balamb BG→Fire Cavern / BG→Balamb Town. Battles auto-escaped with `KEY Z+C 25000`.

## 2026-07-12 playtest chapter (v0.18.3.236–.238, moved from DEVNOTES 2026-07-19; PUSHED, #71–#77 resolved, #71 open low-priority)
...
## .225→.235 chapter (moved from DEVNOTES 2026-07-19; pushed)
...
## v0.18.3.259 (PUSHED, HEAD 49f9d8b) and the #77/#78 field-insert family (moved from DEVNOTES 2026-07-19)

**v0.18.3.259 PUSHED, HEAD 49f9d8b:** #79 exam-car auto-drive CLOSED (vehicleId 0x020409E0); #68 final-approach circle (=1200) killed the entry orbit. Next at the time: #68 mid-route scrapes, offline first.

**#77 closed at .243, #78 closed at .249:** field insert family fully mapped, all resolved in `FieldExpandRawVars` before `Decode()` — **0x04** numbers (value at `0x1D2B4B0+param*4`), **0x0C/0x0D** names (engine resolvers 0x47E970/0x47EA30 via fn ptr), **0x0E+** deferred shared-text table (ptr **0x01D2B80C** = `u16 count; u16 offsets[]; FF8 strings`). **Process rule that came out of this:** two wrong theories → dump the bytes.

## Older shipped chapters digest (moved from DEVNOTES 2026-07-18, v0.18.3.284 session)

Play-by-play in CHANGELOG/HISTORY/closed issues: battle diag #62-#64, #54 status, #48 char-select, PR #26 FMV ADs, #65/#66 Switch screens (`menu_tts_switch.inl`), Timber train hijack #56-#60, status chapter #49/#54 + #10/#42/#46/#47 (`menu_tts_status.inl`). **#67 on-foot drive PUSHED** (main v0.18.3.87+; OPEN as all-continent umbrella; robustness = #68 executor / #69 terrain-ID). The #70 navmesh saga (.104-.124) lives in CHANGELOG.md. OPEN: **#61** (decoder spoken-"L", root `FF8TextDecode::Decode`), **#50** Angelo gauges, **#45** junction non-command abilities (blocked). Handedness: RIGHT=yaw+90 CW.


## GitHub #85 — Deling sewer gates (v0.18.3.286–.292)

**Prior #85 (CHANGELOG `.286`–`.292`):** `.286` tri-centroid + ENT_OBJECT dedup; `.287` SET3 tri-slot marker fix; `.288` `ladline`→"Ladder" — **reverted, it hid the gate**; `.289` Director-promotion widened → `saku1`=Gate 1; `.290` identified `ladline7` as the gate via `[NAV-PROJ] player=`; `.291` fixed 3 self-inflicted defects (incomplete dup-slot guard, SYM leak on table miss, `bg*` over-scoping); `.292` battle-return exit loss → **#88**, `(approx.)` dropped, skip-list applied to JSM objects. Open: `saku4` (Director, positionless by design), `saku5`/`6` (display-only), `hasigo` (promoted, never positioned), `hasigomodel` (spurious 4th "Exit" @(1178,104)).

Full play-by-play per version in `CHANGELOG.md`. Durable lessons were promoted into the DEVNOTES rules section (SYM naming, resolver duplicate-slot guard, field state machines, split-before-you-need-the-room).


**Prior (`.294`) — #37 split, BAT-confirmed inert.** `field_nav_catalog.inl` 81,309→63,017 (→`_triglines.inl`); `field_archive_jsm_scan.inl` 81,184→61,793 (→`field_archive_jsm_classify.inl`). **The classify fragment sits INSIDE `ScanJSMScripts()`'s per-entity for-loop but is brace-balanced and does NOT close it.**


**Prior (`.293`).** varblock 340 as a BYTE = `0` ladder standing / `9` fallen. `ladline7` resolved a REAL position `(-291,799)`, ~16u from our centroid estimate → validates the gate ID AND the centroid fallback. **`[STATE-DIAG]`/`[STATE-GROUP]` live in `RefreshCatalog()` — the catalog must be OPENED for them to log; this has now cost TWO BATs.**
