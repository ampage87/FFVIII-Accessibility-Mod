...
void StartDirectionDrive(int8_t dirX, int8_t dirY, bool walk)
{
    ...
    if (s_directionDriveActive) {
        ...
        // v0.15.9.7.7: Walk vs run uses fundamentally different input paths.
        // When walk=true we use pure keyboard (no fake gamepad, no analog
        // override). When walk=false we use F9-style analog override.
        if (walk != s_directionDriveWalk) {
            // Walk state changed mid-engagement -- swap input paths.
            if (walk) {
                // Run -> walk transition: uninstall fake gamepad, disable override.
                Log::Field("FieldNavigation: [direction-drive] walk-state flip RUN -> WALK: "
                           "uninstalling fake gamepad, disabling analog override");
                s_analogOverrideActive = false;
                s_analogDesiredLX = 0;
                s_analogDesiredLY = 0;
                DD_UninstallFakeGamepad();
            } else {
                // Walk -> run transition: install fake gamepad, enable override.
                Log::Field("FieldNavigation: [direction-drive] walk-state flip WALK -> RUN: "
                           "installing fake gamepad, enabling analog override");
                DD_InstallFakeGamepad();
                s_analogOverrideActive = true;
            }
        }

        // Write analog only when override is active (walk=false path).
        if (s_analogOverrideActive) {
            s_analogDesiredLX = (int32_t)dirX * RUN_ANALOG_MAGNITUDE;
            s_analogDesiredLY = (int32_t)dirY * RUN_ANALOG_MAGNITUDE;
        }
        ...
        if (walk != s_directionDriveWalk) {
            // v0.15.9.7.8: W is NOT an extended key -- pass extended=false.
            // Without this, OS injects malformed scancode E0+11 that FF8
            // doesn't recognize as W.
            InjectKey(SC_W_CANCEL_DD, walk, /*extended=*/false);
    ...
    // Fresh start.
    //
    // v0.15.9.7.7: Walk vs run takes different paths.
    if (walk) {
        // Walk-mode: pure keyboard input. No fake gamepad install, no analog
        // override. Matches Aaron's manual play setup (keyboard arrows + W,
        // no joystick).
        Log::Field("FieldNavigation: [direction-drive] FRESH START walk=1 -- "
                   "keyboard-only path (no fake gamepad, no analog override)");
        s_analogOverrideActive = false;
        s_analogDesiredLX = 0;
        s_analogDesiredLY = 0;
        // (Skip DD_InstallFakeGamepad.)
    } else {
        // Run-mode: F9-style fake gamepad + analog override at full deflection.
        DD_InstallFakeGamepad();
        s_analogOverrideActive = true;
        s_analogDesiredLX = (int32_t)dirX * RUN_ANALOG_MAGNITUDE;
        s_analogDesiredLY = (int32_t)dirY * RUN_ANALOG_MAGNITUDE;
    ...
    if (walk) {
        // v0.15.9.7.8: W is NOT an extended key -- pass extended=false.
        InjectKey(SC_W_CANCEL_DD, true, /*extended=*/false);
        s_directionDriveWalk = true;
        Log::Field("FieldNavigation: [direction-drive] FRESH-START W KEYDOWN "
                   "injected (scancode=0x%02X, extended=0) -- will be held continuously "
                   "until Disengage (no re-press)", (unsigned)SC_W_CANCEL_DD);
    } else {
    ...
    Log::Field("FieldNavigation: [direction-drive] STARTED dir=(%d,%d) walk=%d "
               ...
               (unsigned)arrows,
               (int)s_analogOverrideActive, (int)s_fakeGamepadInstalled);
}
...
void StopDirectionDrive()
{
    ...
    if (s_directionDriveWalk) {
        // v0.15.9.7.8: W is NOT an extended key.
        InjectKey(SC_W_CANCEL_DD, false, /*extended=*/false);
        s_directionDriveWalk = false;
    }
