// menu_tts_save.inl — Save screen TTS (slot/block cursor, LZSS, SETPLACE)
...
void MenuTTS_CacheSaveHeader(const char* filename, const uint8_t* data, int dataLen)
{
    ...
    if (!ParseSaveFilename(filename, &slot, &block)) {
        Log::Menu("[SaveCache] Could not parse slot/block from: %s", filename);
        return;
    }
    ...
    if ((int)compressedSize + 4 > dataLen || compressedSize == 0) {
        Log::Menu("[SaveCache] slot%d_save%02d: bad compressed size %u (file %d bytes)",
                   slot, block, compressedSize, dataLen);
        s_saveCache[si][bi].valid = false;
        return;
    }
    
    Log::Menu("[SaveCache] slot%d_save%02d: file=%d bytes, compressed=%u",
    ...
    Log::Menu("[SaveCache] slot%d_save%02d: decompressed %d bytes",
               slot, block, decompLen);
    
    if (decompLen < 0x50) {
        Log::Menu("[SaveCache] slot%d_save%02d: decompression too short (%d bytes)",
                   slot, block, decompLen);
        s_saveCache[si][bi].valid = false;
        return;
    }
    ...
    for (int i = 0; i + 1 < decompLen; i++) {
        if (decompBuf[i] == 0xFF && decompBuf[i+1] == 0x08) {
            ...
            Log::Menu("[SaveCache]   0x08FF hit #%d at decomp +%d (savemap +%d), lvl_byte=%u",
            ...
            if (candidate >= 0 && candidate + 4 + 32 <= decompLen) {
                ...
                Log::Menu("[SaveCache]     hdr[0..31]: %s", hx);
            }
            hitCount++;
    ...
    Log::Menu("[SaveCache] slot%d_save%02d: %d total 0x08FF hits, using savemap at +%d",
    ...
    if (smOffset < 0) {
        ...
        Log::Menu("[SaveCache] slot%d_save%02d: no valid 0x08FF found. First 32: %s",
                   slot, block, hexOut);
        s_saveCache[si][bi].valid = false;
        return;
    }
    ...
    h->partyStr[0] = '\0';
    {
        ...
        Log::Menu("[SaveCache] slot%d_save%02d: portraits=[%u,%u,%u] party=\"%s\"",
    ...
    Log::Menu("[SaveCache] slot%d_save%02d: \"%s\" lvl=%u loc=%u (%s) gil=%u time=%dh%02dm party=[%s]",
...
static void AnnounceSaveSlot()
{
    ...
    Log::Menu("[MenuTTS] Save slot: %s", slotName);
}
...
static bool CheckSavePhaseTransition(int phaseOffset)
{
    __try {
        ...
        if (s_prevSavePhase >= 2 && phase == 1) {
            ...
            Log::Menu("[MenuTTS] Block->slot transition (+0x%03X: %u->%u), reset for announce",
...
{
    Log::Menu("[SaveScan] === F12 SAVE HEADER MEMORY SCAN ===");
    
    // Pass 1: scan for 0x08FF fixed_value with non-zero data
    Log::Menu("[SaveScan] --- Pass 1: 0x08FF fingerprint ---");
    ...
    for (int i = 0; i < count; i++) {
        ...
        Log::Menu("[SaveScan] FOUND #%d at 0x%08X (pMenuStateA%+d)",
                   i, h->addr, h->relOffset);
        Log::Menu("[SaveScan]   name=\"%s\" lvl=%u loc=%u gil=%u time=%dh%02dm disk=%u",
        ...
        Log::Menu("[SaveScan]   hdr+00: %s", hex1);
        Log::Menu("[SaveScan]   hdr+24: %s", hex2);
    }
    Log::Menu("[SaveScan] Pass 1 complete: %d hits", count);
    
    // Pass 2: scan for "Squall" name pattern in menu encoding
    Log::Menu("[SaveScan] --- Pass 2: Squall name pattern (37 4F 53 3F 4A 4A) ---");
    ...
    for (int i = 0; i < nameCount; i++) {
        ...
        Log::Menu("[SaveScan] NAME #%d at 0x%08X (pMenuStateA%+d)",
                   i, h->addr, h->relOffset);
        ...
        Log::Menu("[SaveScan]   before: %s", hex1);
        ...
        Log::Menu("[SaveScan]   name+: %s", hex2);
    }
    Log::Menu("[SaveScan] Pass 2 complete: %d name hits", nameCount);
    Log::Menu("[SaveScan] === SCAN DONE ===");
}
...
static void PollBlockCursor(int offset)
{
    ...
    __try {
        ...
        if (blockIdx != s_prevBlockCursor) {
            ...
            Log::Menu("[MenuTTS] Block cursor: %s (idx=%u)", buf, (unsigned)blockIdx);
    ...
    if (gcwLen > 0) {
        ...
        if (isSaveScreen) {
            if (!s_saveScreenActive) {
                ...
                Log::Menu("[MenuTTS] Save screen detected (GCW)");
            }
        ...
        if (s_saveScreenActive) {
            ...
            if (s_saveGcwZeroCount >= 20) {
                Log::Menu("[MenuTTS] Save screen exited (no GCW data for %d polls)",
                           s_saveGcwZeroCount);
