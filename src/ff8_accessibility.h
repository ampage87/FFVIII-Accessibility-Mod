...
// refuse to push.
// ================================================================
#define FF8OPC_VERSION "0.18.3.30"  // v0.18.3.30: #59 -- fix "timer still active after the sequence ends" glitch (all timers). The engine global doesn't zero at mission completion; it freezes at the leftover value (the Timber train leaves ~79), so the old deactivate-only-on-raw==0 logic left the mod reporting a phantom running timer. countdown_timer.cpp ACTIVE state now tracks s_lastDecrementTickMs and EnterInactive()s after STALL_TIMEOUT_MS (8s) with no change -- mirror of the decrement-gated activation. FROZEN untouched (held intentionally); a sequence that legitimately pauses the global re-detects on resume.
