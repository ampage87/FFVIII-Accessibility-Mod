...
GitHub: `ampage87/FFVIII-Accessibility-Mod`. **GitHub HEAD = v0.17.8.6** (commit `e415be44`, parent `cb20fd88` v0.17.8.4 — diagnostic builds v0.17.8.2 and v0.17.8.5.x correctly never pushed, history stays linear). Local tree clean at **v0.17.8.6**. Bugs #2, #3, #4, #5, #6 from the Fire Cavern list all closed; the bgryo2_1 dorm-bed + duplicate-exit chapter is closed and pushed.

The v0.17.7.6.x chapter closed the bgroad_5 hallway calibration failure (full narrative in `DEVNOTES_HISTORY.md`); v0.17.8.0 closed bugs #5/#6, v0.17.8.1.1 closed #3, v0.17.8.3 closed #4, v0.17.8.4 removed a bogus camera catalog entry, v0.17.8.6 added the dorm bed + killed its duplicate exit. **Current chapter: v0.17.8.7 — phantom interactive objects (the `cardmaster` debug leftover on bgroad_5) + the runtime dialog-confirmation/disk-persistence layer.** Still deferred: Laguna dream bugs #7 (field-nav player detection) and #8 (battle announces wrong party).
