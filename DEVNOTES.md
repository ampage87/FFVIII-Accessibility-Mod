...
**Ability Build 1 (ability-list phase) COMPLETE & CONFIRMED — v0.18.1.3, ABIL_DIAG=0, READY TO PUSH (Aaron runs `push_to_github.ps1`).** `src/menu_tts_ability.inl` (included after `menu_tts_gf.inl`) + `PollAbilitySubmenu()` dispatch on top-level cursor 5 + `/`-help via `AbilitySpeakSelectedHelp()`. Confirmed in-game: ability NAME on `+0x258` move; `/` reads help; empty padded rows (cursor `count..63`, blank help) say "Empty Ability Slot" and `/` repeats it. List read from GCW by forward-scanning the longest run of menu-ability names (ids 97–115) — NO menu-token anchor (the bar scrolls: "GFAbilitySwitch...Save"; the original `rfind("Junction")` gave count=0). Reuses `GcwAbilityNames()`. Live: count=2 ids=[98,108], cursor 0–10 (rows 0–1 real, 2–10 empty). Gate `+0x1E8==14`, phase `+0x22E==3`. Issue #42 rewritten to the "Use GF ability" model. **After push: update HEAD here + verify via `github:list_commits`.** Next: Build 2 (refine item list, `+0x22E ~19–21`, cursor `+0x2DF`): item name + qty + greyed state, `/` for refine info.
