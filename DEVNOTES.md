...
**★ LATEST (v0.18.3.254 — BAT PASSED; draw-point chapter COMPLETE: #78/#80/#81 all CLOSED. .244–.254 chain PUSH-READY, Aaron pushes; GitHub HEAD = .243, verify).** Draw points = engine-HARDCODED dialogs (GETSTR + show_dialog paths only, NO opcodes) and they populate the STANDARD cursor fields 0x29/0x2A/0x2B ([CHOICEQ] proved firstQ=1 lastQ=2; 0xFF sentinel on close). **#80 final stack (drain):** live re-read via rawPtr; reverse containment (fixed msgId=4/6); insert-WILDCARD structural match with TrimDecoded edge chunks (fixed msgId=2). Banked: message bytes never mutate; insert source is RENDER-TRANSIENT (decodes populated only inside the render pass). **#81:** PollWindows drives ScanAndSpeakChoiceWindows with QUIET-INIT ("Selected: <option>" when show_dialog already spoke the plain text); .254 BAT: quiet-init + per-move "Choice changed" announces all confirmed, zero double-speak. POLL_CHOICE_DIAG [CHOICEQ] still on — gate off in the next real build.
