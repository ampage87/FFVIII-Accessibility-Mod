...
**#69 (.88-.95) PUSHED -- main=v0.18.3.95, commit c591803b** (squashed onto .87). Core = .90 height-step guard: per-cell s_elevFine; PlanPathFine blocks an inter-cell edge when |elevA-elevB| > WM_CLIMB_STEP=400 (road-to-road EXEMPT; false-coast cliff ~1513-1570 so 400 is well-centred). .81 + .85 KEPT load-bearing, only offroad retired; #69 stays OPEN for future geometry terrain-ID. Files: state/segments/planner.inl. **TRIANGLE-NAVMESH PIVOT -- PORTED + BAT'd v0.18.3.104 (LOCAL, validated).** Coarse-grid heuristic (1024u raster + .81/.85 patches) is fragile -- fixing Dollet broke Timber. The RAM-read of runtime polygon adjacency (`[0x020402DC]`) is ABANDONED (.101/.103 disproved the descriptor model; RAM reads are build-fragile anyway). PIVOT (Aaron approved): TRUE TRIANGLE NAVMESH from the wmx tris LoadTerrainGrid parses. **Offline prototype VALIDATED on real wmx.obj** (Python + host-compiled C++): 157,416 tris; exact shared-edge adjacency alone = 642 islands; axis-aligned T-junction bridging -> 253 comps, largest 74,308; Galbadia gated flood (gate 400) reaches Dollet/Timber/G-Garden/Deling, not Balamb (ocean); A* Galbadia->Dollet = 30,259u/126 tris. **PORTED .104 NON-INVASIVELY** (gated `NAVMESH_DIAG`, LOCAL): new `world_map_navmesh.inl` (host-compilable; container-tested `tests/test_navmesh.cpp`, 0 fails); LoadTerrainGrid feeds each non-ocean tri (same verts+guard as RasterizeTriFine) -> Build -> Initialize logs connectivity; planner/executor UNTOUCHED. BAT .104 DONE -- in-game `[NAVMESH]` == offline TO THE DIGIT (157416/253/74308; Dollet/Timber/G-Garden/Deling REACHABLE, Balamb not; A* 30259/126). **Dollet wedge = false coast ~4 cells WEST of the .81 box:** player engine-walled at fine(100,65) where the grid says walkable + ON-ROUTE (slides E-W, free S, blocked N). So **.81 box (cols 104-111) is MISPOSITIONED** -- misses the real wall at col 100; neither .81 nor the navmesh swap fixes it (navmesh also blind to the false coast, Finding 4). **Durable fix = DISASSEMBLY track** (recover the engine's per-edge walkmesh build). Pending: close #67 + BAT 3 coord audit. Backlog #61/#50/#45/#37/Irvine (hold). (OneDrive macro-revert risk: re-sync ff8_accessibility.h if V-key/[PLAN] version != CHANGELOG top.)
