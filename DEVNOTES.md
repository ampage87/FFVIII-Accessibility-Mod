...
**Latest PUSHED = `b342a5df` = v0.18.3.37 — main-menu Switch submenu (#65) SHIPPED & CLOSED; #66 (forced party-select story screens) opened as the follow-up.** The `.34`–`.37` work + the `.32` CHANGELOG-heading restore came over as one commit on `c9a4c777` (v0.18.3.33). Battle diagnostic captures all gated (#62/#63/#64 closed). Prior shipped: #54 status (v0.18.2.50), #48 char-select, PR #26 FMV ADs. Play-by-play in DEVNOTES_HISTORY + CHANGELOG.

**#65 Switch submenu TTS — SHIPPED & CLOSED** at v0.18.3.37 (`b342a5df`). **ACTIVE CHAPTER: #66 — forced party-select / junction-switch story screens** (follow-up). The forced screen reuses the Switch Member UI (screenshots confirm) but runs in game mode 10 without the main-menu context (`PollSwitchSubmenu` gates on `s_prevCursor==6`). Reuse `menu_tts_switch.inl`. **Switch offsets** (full detail in CHANGELOG v0.18.3.34–.37 + closed #65; reused by #66): `+0x1E8==10`, focus `+0x22E` (12=bar/2=list), option `+0x25E` (0=Switch/1=Exchange), `<Name>LVHP` GCW tokens, roster `+0x1DB`, HP via compStats `0x1CFF000`(active)/`+0x71E`(benched), level via `ComputeCharLevel` (EXP/1000+1). **#66 LOCAL = v0.18.3.48 — forced party-select; BAT PENDING.** Switch Member screen in **GAME MODE 10** (`PollForcedPartySelect`, `menu_tts_switch.inl`). **.44/.45 BAT-confirmed:** live working party at `+0x1EA` (3 active, 0xFF=empty; reserves `+0x1ED`; `+0x1EC`=slot3 confirmed); empty active="Empty Party Slot" / empty reserve="Empty Reserve Slot" (reserve id from `+0x1ED` by cursor `+0x1E9`, 2×4 grid 0-7; `ForcedPselReserveId`); option from authoritative `+0x1E6` (0=Switch/1=Junction). **.47/.48:** `.47` announces the focused char when focus drops from the bar onto a char (CONFIRMED working). `.48` adds the mirror: `movedToBar` (onBar && prev focus==2) → speak the current option name. Both via FOCUS byte `+0x1B6` (2=char grid, 0x0C=bar, 0x0B transient; `ForcedPselFocus()`; sat just below the old diag window — widening to +0x100..+0x500 found it). `FORCED_PSEL_DIAG`=1 (confirm .48). **BAT:** win → from a char go Up to the bar (announces option) → flip Switch↔Junction (option name) → Down onto a char (char announces). If clean: reserve id→name off `+0x1ED`, diag off, push. Open backlog: #61, #50-53, Irvine Shot (on hold).
