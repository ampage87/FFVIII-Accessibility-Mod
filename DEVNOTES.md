...
**Ability Build 2 (refine item-list phase) WRITTEN — v0.18.1.4, awaiting BAT (local, NOT pushed).** Build 1 (ability list) is PUSHED at `af7bf5b`; Build 2 adds the refine ITEM-LIST phase to `src/menu_tts_ability.inl`. `PollAbilitySubmenu` now branches on `+0x22E`: ability list (==3, unchanged) vs item list (>=19 → `PollAbilityItemList`). Item handler announces the highlighted source item's NAME + QUANTITY on `+0x2DF` move (read from savemap inventory via `AbilReadInvSlot`, mirroring `AnnounceItemAtCursor`: `inv[cur*2]`=id, `inv[cur*2+1]`=qty; empty/over-range → "Empty"); `/` reads the refine preview ("N will refine into M <Magic>") parsed from the GCW by `ParseRefinePreview` (anchors "will refine into", back over leading digits, stops at first party-name HELP_END_MARKER). `ABIL_DIAG=1`: `[ABILDIAG-ITEM]` logs cur + savemap id/qty/name + GCW to confirm the **item-list==full-inventory-in-order** assumption — if a name mismatches the screen, pivot to GCW item-name parse + qty-by-id. Flip ABIL_DIAG to 0 once confirmed, then bump+push. Deferred to Build 2b: greyed/non-refinable state, pagination for long lists. Next: BAT → read build_latest.log (expect v0.18.1.4) + ff8_menu.log `[ABILDIAG-ITEM]`.
