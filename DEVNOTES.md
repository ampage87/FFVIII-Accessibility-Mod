...
**PUSHED HEAD = v0.18.3.52 (`74bc49a2`) — #67 step 1:** world-map coord/segment/BFS math extracted to new `src/world_map_geometry.inl` (no Win32/SEH; included after state.inl) + new `world-map-harness` CI job. Behavior-preserving, BAT-confirmed.

**#65 + #66 Switch screens — DONE / PUSHED v0.18.3.51 / #66 CLOSED.** Main-menu Switch (mode 6) + forced party-select (mode 10) on one `PollSwitchScreen(state, off)` in `menu_tts_switch.inl` (off=0 forced / 0x78 main, gated `s_prevCursor==6 && +0x1E8==10`). Offsets in closed #66 + CHANGELOG.

**#67 World-Map on-foot auto-drive to Dollet — SOLVED (v0.18.3.87); READY TO PUSH.** Arc: fine 256x192 reachability + slope mountains (.55); clearance-weighted Dijkstra planner. ROOT = the coarse 1024u grid guesses walkability, wrong at cliffs. PIVOT (Aaron) = ROAD-based nav (wmx road/railroad terrain 27/28). **.84-.86 SOLVED ROUTING** (road extraction -> walkable override -> road-PREFERENCE in PlanPathFine: road step cost 1 no-clearance-penalty, non-road pays clearance+flat WM_OFFROAD_PENALTY=40, so the route FOLLOWS the road; Balamb unchanged). **.87 SOLVED THE EXECUTOR = YAW-BASED SCREEN-RELATIVE 8-WAY steering:** on foot hdg(0x0203ED02) is the FIXED per-region CAMERA YAW, not a steerable facing, so .80's heading turn-then-go could only press pure UP (= walks at the yaw) and drifted off the thin road into the cliff; instead screenAngle=(targetBearing-yaw)&0xFFF and press the nearest of 8 arrow combos (sector=((screenAngle+256)>>9)&7) so he STAIRCASES along the road. On-foot RE-SPLIT off the vehicle law (new else-if(isOnFoot); vehicles untouched; dead if(false&&isOnFoot) left dead). **.87 BAT'd -> ARRIVED at Dollet** (slow last ~3km final approach = minor future polish, left alone). **SHIP STATE:** all 5 #67 diags now OFF (DRIVE_STEER_DIAG/ROUTE_MAP_DIAG/ROAD_MAP_DIAG/WM_CALIB_DIAG/CAMERA_SCAN_DIAG) -- all call sites #if-guarded, TriggerCameraScan keeps its #else stub, road overlay/route/steering unconditional, so logging-only with no behavior change. **NEXT:** one final rebuild-and-confirm BAT (clean build diags-off + still reaches Dollet + Balamb regression), then Aaron pushes .54-onward as ONE commit (main v0.18.3.52 `74bc49a2` -> v0.18.3.87), then close #67, then **BAT 3** = world-map catalog coord audit. **Then #61**; backlog #50/#45/#37/Irvine Shot (hold). Handedness note if revisited: RIGHT=yaw+90 CW (swap R<->L if a foot drive tracks bends wrong).
