...
10. **B-Garden Hall 6 (`bghall_3`, field 170) — NPC Xu labeled "Interaction 3" — IN PROGRESS.** Xu = JSM `kanban2` (ent25, cat3, PSHM pos (4626,-3459)); confirmed a real walk-up-and-Confirm NPC (dialog log "Xu …"; F11 `f11_220226_490.png` shows her character model standing on the walkway). The old "Director over-promotion / shu" hypothesis is FALSIFIED. A [npc-class] read-only diagnostic (v0.17.8.9-diag, removed) proved the static signals CANNOT separate Xu from props: talkSetup=0 for EVERYTHING (her talk is runtime-dispatched via 0x1C; no static TALKRADIUS), and setmodelInit=1 for Xu AND `water` AND `walllight`. So neither hasTalkSetup nor setmodelInit is a discriminator. KEY: the runtime classifier already labels live character models correctly — this run had `ent5 model=15` → "NPC" (cat1) alongside kanban2's "Interaction 3" (cat7). The ONLY thing separating Xu from a sign/light/water is the MODEL itself, which the JSM scan does not currently capture. PLAN (next change): capture SETMODEL's model-id operand into JSMEntityInfo; diagnostic-dump it + the runtime others' SYM/model/pos to confirm kanban2 loads a character-range model (≥10, like ent5's 15) vs prop ids; then classify a cat-3 Interactive Object with a character-range model as NPC (surfaced as a person before approach, no cache). Also watch for a possible Xu DUPLICATE (runtime "NPC" + JSM "Interaction 3") when she's active — dedupe by position.
